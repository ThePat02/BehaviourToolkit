...
enum CallTarget { ACTOR, BLACKBOARD, CUSTOM }  ## The actor node set on the BTRoot node.  ## The blackboard node set on the BTRoot node.  ## A custom node set on the custom_target variable.
