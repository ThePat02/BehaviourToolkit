...
func _parse_property(object, type, name, hint_type, hint_string, usage_flags, wide):
	...
	if object is LeafCondition:
		...
		if name == "condition_value_string":
			return (
				not value_type == LeafCondition.ConditionValue.STRING
				or condition_type == LeafCondition.ConditionType.CUSTOM_SCRIPT
			)

		if name == "condition_value_int":
			return (
				not value_type == LeafCondition.ConditionValue.INT
				or condition_type == LeafCondition.ConditionType.CUSTOM_SCRIPT
			)

		if name == "condition_value_float":
			return (
				not value_type == LeafCondition.ConditionValue.FLOAT
				or condition_type == LeafCondition.ConditionType.CUSTOM_SCRIPT
			)

		if name == "condition_value_bool":
			return (
				not value_type == LeafCondition.ConditionValue.BOOL
				or condition_type == LeafCondition.ConditionType.CUSTOM_SCRIPT
			)
